// Example of using gaussian process for the time trend
data {
  int<lower=1> N;
  int<lower=1> M;
  int<lower=1> K;
...
transformed parameters {
  ...
  vector[M] gp;
  { // local scope to avoid saving intermediate quantities
...
generated quantities {
  ...
  for (n in 1:N) {
    y_rep[n] = neg_binomial_2_log_rng(
      mu[building_idx[n]] + kappa[building_idx[n]] * traps[n] + gp[mo_idx[n]] + log_sq_foot[n],
      phi
    );
  }
}
